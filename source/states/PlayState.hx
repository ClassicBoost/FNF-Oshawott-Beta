...
class PlayState extends MusicBeatState
{
	...
	public var choosenfont:String = 'vcr.ttf';
	...
	public var disableGhost:Bool = false;
	...
	override public function create()
	{
		...
		if (stageData.stageUI != null && stageData.stageUI.trim().length > 0)
			...
			if (stageData.isPixelStage) {
				stageUI = "pixel";
				choosenfont = 'vcr-og.ttf';
			}
		}

		switch (Paths.formatToSongPath(SONG.song)) {
			case 'ugh', 'oshawugh', 'call', 'call-shiny', 'oshawugh-shiny','figure-faceoff','figure-faceoff-old','figure-faceoff-shiny','figure-faceoff-shiny-old':
				choosenfont = 'vcr-og.ttf';
			case 'oshano','oshano-shiny':
		...
		timeBar = new HealthBar(0, timeTxt.y + (timeTxt.height / 4), 'UI/timeBar', function() return songPercent, 0, 1);
	...
	//	add(timeBar);
		...
		healthBar = new HealthBar(0, FlxG.height * (!ClientPrefs.data.downScroll ? 0.89 : 0.11), 'UI/healthBar', function() return health, 0, 2);
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				switch (swagCounter)
				{
					case 0:
						...
						tick = THREE;
					case 1:
						countdownReady = createCountdownSprite('UI/' + introAlts[0], antialias);
						FlxG.sound.play(Paths.sound('intro2' + introSoundsSuffix), 0.6);
						tick = TWO;
					case 2:
						countdownSet = createCountdownSprite('UI/' + introAlts[1], antialias);
						FlxG.sound.play(Paths.sound('intro1' + introSoundsSuffix), 0.6);
						tick = ONE;
					case 3:
						countdownGo = createCountdownSprite('UI/' + introAlts[2], antialias);
	...
	public function updateScore(miss:Bool = false)
	{
		...
		if(totalPlayed != 0)
		{
			...
			str = '$percent%${ratingFC != '' ? ' [$ratingFC]' : ''}';
		}

		scoreTxt.text = 'Score: ' + songScore
		+ divider + 'Accuracy: ' + str
		+ (songMisses > 0 ? (divider + 'Misses: ' + songMisses) : '')
		+ divider + 'Rank: ' + ratingName
		+ (practiceMode == true ? '${divider}Practice Mode' : '');
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.ONE) {
			...
			switch (Paths.formatToSongPath(SONG.song)) {
				case 'oshano':
					PlayState.SONG = Song.loadFromJson("oshano-shiny", "oshano-shiny");
					FlxG.switchState(new PlayState());
				case 'oshano-shiny':
					// do nothing
				default:
					PlayState.SONG = Song.loadFromJson("oshano", "oshano");
					FlxG.switchState(new PlayState());
			}
		}
		...
		iconP1.setGraphicSize(Std.int(FlxMath.lerp(iconP1.width, 150, 0.09 / (ClientPrefs.data.framerate / 60))));
		iconP2.setGraphicSize(Std.int(FlxMath.lerp(iconP2.width, 150, 0.09 / (ClientPrefs.data.framerate / 60))));
	...
	{
		if (Paths.formatToSongPath(SONG.song) == 'oshano' || Paths.formatToSongPath(SONG.song) == 'oshano-shiny') {
			// fuck you
			MusicBeatState.resetState();
			PauseSubState.itemUse = 'lmao';
		} else {
	...
	function displayRating(daRatingLol:String, negative:Bool = false) {
		...
		rating.loadGraphic(Paths.image('UI/' + uiPrefix + 'ratings/' + (allSicks == true && !negative ? 'sick-perfect' : daRatingLol) + daTiming + uiSuffix));
		...
		{
			var numScore:FlxSprite = new FlxSprite().loadGraphic(Paths.image('UI/' + uiPrefix + 'combo/${(allSicks == true && !negative ? 'golden/' : '')}num' + Std.int(i) + uiSuffix));
	...
	private function keyPressed(key:Int)
	{
		if (!cpuControlled && startedCountdown && !paused && key > -1)
		{
			if(notes.length > 0 && !boyfriend.stunned && generatedMusic && !endingSong)
			{
				...
				var canMiss:Bool = (!ClientPrefs.data.ghostTapping || disableGhost);
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		noteMissCommon(daNote.noteData, daNote, !daNote.isSustainNote);
	...
	{
		if(ClientPrefs.data.ghostTapping && !disableGhost) return; //fuck it
	...
	function noteMissCommon(direction:Int, note:Note = null, ?stackMisses:Bool = true)
	{
		// score and data
		...
		} else healthCall(-0.02 * healthLoss);
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (!note.isSustainNote)
			{
				...
				if (ifMiss != '' && ClientPrefs.data.lateDamage) {
					FlxG.sound.play(Paths.soundRandom('missnote', 1, 3), FlxG.random.float(0.1, 0.2));
					songMisses++;
					if (combo > 0) combo = 0;
					combo--;
			...
			if(!note.noAnimation) {
				...
				if(char != null)
				{
					char.playAnim(animToPlay + note.animSuffix + ifMiss, true);
					char.holdTimer = 0;
	...
	public function healthCall(number:Float) {
		health += number;
	}
	...
	override function stepHit()
	{
		...
		PauseSubState.itemUse = 'none';

		switch (Paths.formatToSongPath(SONG.song)) {
			case 'ocean-aggrivation-shiny', 'figure-faceoff-shiny':
				disableGhost = true;
			case 'dance':
				health = 2;
			case 'oshano','oshano-shiny':
	...
	override function beatHit()
	{
		...
		if (Paths.formatToSongPath(SONG.song) == 'lore') {
			if (curBeat % 2 == 0) {
				iconP1.flipX = false;
				iconP2.flipX = false;
			} else {
	...
	function fullComboUpdate()
	{
		...
		if (songMisses < 10)
			ratingFC = 'SDCB';
		if (songMisses >= 65 && songMisses < 500)
			ratingFC = 'Skill Issue';
		if (songMisses >= 500 && songMisses < 1000)
			ratingFC = 'Bro okay?';
		if (songMisses >= 1000)
			ratingFC = 'What song are you playing? wtf';
		}
	}
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled && Achievements.getAchievementIndex(achievementName) > -1) {
				...
				else
				{
					switch(achievementName)
					{
						...
						case 'oversinging':
							unlock = (boyfriend.holdTimer >= 10);

						case 'hype':
							unlock = (!boyfriendIdled);

						case 'two_keys':
							unlock = (keysPressed.length <= 2);
